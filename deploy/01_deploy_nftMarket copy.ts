...
const deployFunction: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }) => {
  ...
  const args: string[] = []
  const NFTMarketPlace = await deploy("NFTMarket", {
    from: deployer,
    args: args,
    log: true,
  ...
  log(
    `npx hardhat read-keepers-counter --contract ${NFTMarketPlace.address} --network ${networkName}`
  )
  log("----------------------------------------------------")
}
...
deployFunction.tags = [`all`, `nftmarket`]
